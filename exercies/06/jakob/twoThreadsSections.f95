program predPreySpace
  ...
  do counter = 2, steps
    !$OMP PARALLEL SECTIONS num_threads(4)
      !$OMP SECTION
        diffPartPrey = MATMUL(diffusionMatrix, preyPop_last)
      !$OMP SECTION
        diffPartPred = MATMUL(diffusionMatrix, predPop_last)
      !$OMP SECTION
        interactPartPrey = preyPop_last*(alpha - beta*predPop_last - lambda*preyPop_last)
      !$OMP SECTION
        interactPartPred = predPop_last*(delta*preyPop_last - gamma - mu*predPop_last)
    !$OMP END PARALLEL SECTIONS

    ! Explicit Euler and bookkeeping
    predPop_last = predPop_last + dt*(diffPartPred + interactPartPred)
    predPop(counter,:) = predPop_last(:)
    preyPop_last = preyPop_last + dt*(diffPartPrey + interactPartPrey)
