program predPreySpace
  ...
  do while (counter <= steps)
    ...
    if (MOD(counter, save_every_nth) == 0 .AND. (counter/save_every_nth) <= steps) then
       predPop((counter/save_every_nth),:) = predPop_current(:)
       preyPop((counter/save_every_nth),:) = preyPop_current(:)
       times((counter/save_every_nth)) = t0 + (DFLOAT(counter)*dt)
       final_counter = counter/save_every_nth
        ! write(*,*) "counter:", counter, "counter/save_every_nth:", &
        !  (counter/save_every_nth), "time:", times((counter/save_every_nth))
     end if
  ...
  open(unit = 20, file = 'results.csv', action = 'write')
  ...
  do while (counter <= steps/save_every_nth .AND. counter < final_counter)
    ...
    ! write(*,*) "Writing time:", counter, times(counter)
    counter1 = 1
    do while(counter1 < N)
